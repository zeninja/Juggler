...
public class BounceControllerDragDownLockedWithRotation : MonoBehaviour {
	...
	void Update() {
		...
		if (Input.GetMouseButton(0)) {
			...
			transform.position = new Vector2(dragEnd.x, 0);
	...
	void UpdateLine() {
		...
		Vector3[] linePositions = new Vector3[] { (Vector2)transform.position + throwForce * lineLengthModifier, transform.position };
		lineRenderer.SetPositions(linePositions);

		float distance = Mathf.Abs(throwForce.magnitude);
	...
	void UpdateArrow() {
		...
		arrow.transform.position = (Vector2)transform.position + throwForce * lineLengthModifier;

		Vector3 diff = throwForce;
	...
	void ThrowBall() {
		// Throw the ball
		if(ball != null) {
			ball.GetComponent<Rigidbody2D>().velocity = throwForce * throwForceModifier;
		}
//		Reset();
	}
